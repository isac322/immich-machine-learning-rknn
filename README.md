...
5. Applies `patches/pr-23958-rknn-shared-weights.patch`.
6. If `git apply --check` fails → workflow fails loudly. No silent rollback. Refresh patch then retry.
7. Builds `machine-learning/Dockerfile` `--target prod-rknn` on `ubuntu-24.04-arm` (GitHub-hosted ARM64 runner, free for public repos).
...
git apply --3way ../patches/pr-23958-rknn-shared-weights.patch  # try first
...
git diff --staged "$TAG" > ../patches/pr-23958-rknn-shared-weights.patch
cd ..
# Verify on a fresh clone:
rm -rf verify
git clone --depth 1 --branch "$TAG" https://github.com/immich-app/immich.git verify
git -C verify apply --check patches/pr-23958-rknn-shared-weights.patch
